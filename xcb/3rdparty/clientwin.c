...
Window_Is_Viewable(xcb_connection_t *dpy, xcb_window_t win)
{
    ...
    if (xwa) {
        ok = (xwa->_class == XCB_WINDOW_CLASS_INPUT_OUTPUT) && (xwa->map_state == XCB_MAP_STATE_VIEWABLE);
        free(xwa);
    }
...
Find_Roots(xcb_connection_t *dpy, xcb_window_t root, unsigned int *num)
{
    ...
    *num = 0;
    ...
    if ((prop_reply->value_len > 0) && (prop_reply->type == XCB_ATOM_WINDOW)
        ...
        prop_ret = malloc((size_t)length);
        if (prop_ret) {
            memcpy(prop_ret, xcb_get_property_value(prop_reply), (size_t)length);
